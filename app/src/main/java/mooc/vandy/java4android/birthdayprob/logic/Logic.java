...
public class Logic 
    ...
    */
    ...
    public double calculate(int size, int count) {
        ...
        int matchingSimulations = 0;
        int[] matchingBirthdayCount = new int[365];
        Arrays.fill(matchingBirthdayCount, 0);
        ...
        for (int i=0; i<count; i++){
            ...
            for (int j=0; j<size; j++){
                int birthdayEachPerson = generator.nextInt(365);
                //adding count to birthdays
                if(matchingBirthdayCount[birthdayEachPerson] != 0) {
                    matchingSimulations++;
                    break;
                }
                else {
                    matchingBirthdayCount[birthdayEachPerson]++;
                }
            }
            Arrays.fill(matchingBirthdayCount, 0);
        }

        double percentageOfTwoBirthdaysOnSameDay = matchingSimulations*100.00/count;
        return percentageOfTwoBirthdaysOnSameDay;
    }
